...
//        saver.setDestination(new File(arff));
    ...
    public void testUsingModels(File file) throws Exception {
        if(modeloBayes==null && modeloJ48==null && modeloForest==null){
            throw new Exception("Debe cargar/crear al menos uno de los tres modelos para poder probar");
        }
        ArffLoader loader = new ArffLoader();
        loader.setFile(file);
        Instances data = loader.getDataSet();
    ...
    public void clasifyUnclasiffiedInstances(File file) throws Exception {
        if(modeloBayes==null && modeloJ48==null && modeloForest==null){
            throw new Exception("Debe cargar/crear al menos uno de los tres modelos para poder clasificar");
        }
        ...
        for (int i = 0; i < unlabeled.numInstances(); i++) {
            ...
            double clasificationJ48 = modeloJ48==null?-1L: modeloJ48.classifyInstance(labeledJ48.instance(i));
            double clasificationBayes = modeloBayes==null?-1L:modeloBayes.classifyInstance(labeledBayes.instance(i));
            double clasificationRForest = modeloForest==null?-1L:modeloForest.classifyInstance(labeledRndForest.instance(i));
...
//            System.out.println(labeledJ48.classAttribute().value((int) clasificationJ48));
            ...
            //Solo se somete a votación cuando se usan los 3 modelos
            if(clasificationJ48>=0 && clasificationBayes>=0 && clasificationRForest>=0){
        ...
        if(modeloJ48!=null){
            String fileName = file.getAbsolutePath().substring(0, file.getAbsolutePath().lastIndexOf(".")) + ".classifiedJ48.arff";
        ...
        if(modeloBayes!=null){
            String fileName = file.getAbsolutePath().substring(0, file.getAbsolutePath().lastIndexOf(".")) + ".classifiedBayes.arff";
        ...
        if(modeloForest!=null){
            String fileName = file.getAbsolutePath().substring(0, file.getAbsolutePath().lastIndexOf(".")) + ".classifiedRFoest.arff";
        ...
        if(modeloBayes!=null && modeloForest!=null && modeloJ48!=null){
            String fileName = file.getAbsolutePath().substring(0, file.getAbsolutePath().lastIndexOf(".")) + ".classifiedDefinitive.arff";
            String csvFileName = file.getAbsolutePath().substring(0, file.getAbsolutePath().lastIndexOf(".")) + "_classified.csv";
            frame.appendMessage("Archivo definitivo: "+fileName);
            frame.appendMessage("\n");
            frame.appendMessage("Archivo definitivo (.CSV): "+csvFileName);
            frame.appendMessage("\n");
            CSVSaver saver = new CSVSaver();
            saver.setInstances(labeledDefinitive);
